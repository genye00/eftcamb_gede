...
module EFTCAMB_main
    ...
    type, extends(TCambComponent) :: TEFTCAMB
        ...
        ! EFTCAMB delta_phi perturbation flags:
        logical   :: EFT_evolve_delta_phi          !< Flag that decides whether integrate delta_phi instead of pi for covariant theories.
        logical   :: EFT_evolve_metric_h           !< Flag that decides whether integrate h' instead of using constraints.
        logical   :: EFT_skip_stability            !< Flag that decides whether skip all stability checks.
...
contains
    ...
    subroutine read_EFTCAMB_flags_from_file( self, Ini )
        ...
        ! delta_phi equation flags, currently only supported by the Horndeski module
        self%EFT_evolve_delta_phi         = Ini%Read_Logical( 'EFT_evolve_delta_phi', .false. )
        if ( self%EFT_evolve_delta_phi .and. ( self%EFTflag /= 4 .or. self%FullMappingEFTmodel /= 0) ) then
            write(*,*) 'EFTCAMB Error: Evolving delta_phi equation is currently only supported for the Horndeski module (EFTflag = 4 and FullMappingEFTmodel = 0).'
            call MpiStop('EFTCAMB error')
        end if
        self%EFT_evolve_metric_h          = Ini%Read_Logical( 'EFT_evolve_metric_h', .false. )
        self%EFT_skip_stability           = Ini%Read_Logical('EFT_skip_stability', .false.)
                ...
                select case ( self%FullMappingEFTmodel )
                    ...
                    case default
                        if (self%EFTCAMB_feedback_level > 0) then
                            ...
                            write(*,'(a)')    'Please select an appropriate model:'
                            write(*,'(a)')    'FullMappingEFTmodel=0  General Horndeski'
